...
- Старих JSON-файлів (`bearingsTable3.json` тощо) більше немає — видалені 2026-08-28, дані живуть у БД
...
4. **3D модель** — перевірити `public/models/` чи є `{ARTICLE-SLUG}.glb`:
   ...
   - **⛔ СТОП** — власник перевіряє картку: якщо є 3D модель — підтвердити що вона відображається і працює коректно; якщо немає — підтвердити стандартний шаблон
5. Описи — власник надає uk-текст, переклад en/pl робить асистент. **Перед внесенням обов'язково:**
   - Перевірити граматику (uk/en/pl)
   - Звірити всі числові значення (d, J, L, B, Cdyn, Co, Pu, mass) з `product_specs` у БД — жодного розходження
   - **⛔ СТОП** — вивести виправлений текст у чат, власник підтверджує
